  ...
  void forward_prop() {
    for (int i = 1; i < num_layers; i++) {
        ...
        for (int j = 0; j < num_neurons[i]; j++) {
            ...
            if (i < num_layers - 1)
                lay[i].actv[j] = fmaxf(0, sum);  // ReLU
            else {
                float z = fminf(fmaxf(sum, -100.0f), 100.0f);  // Evitar overflow
                lay[i].actv[j] = 1.0f / (1.0f + expf(-z));     // Sigmoid
            }
        }
    }
}
    ...
    for (int i = 0; i < num_layers - 1; i++) {
        ...
        for (int j = 0; j < num_neurons[i + 1]; j++) {
            for (int k = 0; k < num_neurons[i]; k++) {
                lay[i].out_weights[j * num_neurons[i] + k] -= (alpha * lay[i].dw[j * num_neurons[i] + k]);
            }
        }
