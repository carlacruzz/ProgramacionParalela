...
void train_neural_net() {
    ...
    #pragma acc data copyin(lay[:num_layers], num_neurons[:num_layers], desired_outputs[:num_training_patterns][:num_out_layer], input[:num_training_patterns][:num_neurons[0]]) \
                 copy(lay[0].out_weights[:], lay[0].bias[:]) 
    {
...
void test_nn() {
    ...
    #pragma acc parallel loop copyin(rSet[:num_test_patterns][:num_neurons[0]]) copy(lay[:num_layers])
